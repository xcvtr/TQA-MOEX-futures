...
**Параметры:** риск-ориентированный sizing (10% капитала на сделку), комиссия 4×2 RUB, RiskManager (DD-stop 20%, max 5 concurrent). Контракты — без капов, только расчёт от капитала и GO.

```
Капитал: 100,000 → 1,189,251,709 RUB (+1,189,151%)
MDD: 60.6% | Сделок: 107 | WR: 65.4% | Calmar: 19,629
```
...
| Si (USDRUB) | 1 | StopHunt + CVD + LunchRev |
...
3. **Churn отключён** — WR 58.6% но отрицательный PnL (асимметрия win/loss).
4. **MDD 33.5%** — превышает дефолтный RiskManager (20%). Нужна калибровка.
