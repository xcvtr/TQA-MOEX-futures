...
**Последний чекпойнт: 222 (2026-08-09)** — Новая стратегия: **базис-арбитраж Si/Eu/CNY ↔ перпетуалы (USDRUBF/EURRUBF/CNYRUBF)**. КСУР-ГО в 16× меньше полного → SHORT-only z>1.5 + ТРИЗ (фильтр ср/чт/пт, risk 6%, lots 30-50): **CAGR 59-72%, DD 12-17%, Calmar 4.3-4.8**. Портфель OI+BA (общий капитал): **pyr=2 risk=5% lots≤1000 → CAGR 1831%, MTM 15%, Calmar 122** (компаунд микро-капитала; реалистично OI 60-220%/год). Ликвидность проверена: mt5 vol = tick_volume, реальные объёмы в 10-50× больше (BR 631K/день) — рынок вмещает всё. Реопт OI теперь с MTM-фильтром ≤20%. **Live-исполнение = MT5 (FINAM) или Finam API — мост ещё писать.** ТРИЗ: 2/7 идей; futoi/M15/TP/ЦБ/Si-Eu отброшены. AlfaForex отброшен (своп −500, нет lead-lag). Скрипты: `scripts/basis_arb/`. **Базис НЕ в live — ждёт решения.** Ранее: чекп.221 — OI NG/BR/SV live-эдж (MC p=0.0000, 60-220%/год, MDD 13-29%).
