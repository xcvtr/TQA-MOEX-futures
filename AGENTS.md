...
**Последний чекпойнт: 223 (2026-08-09)** — Live реинвест/компаунд починен: 3 лимита душили рост лотов (volume cap 0.2×tick_volume → макс 11 лотов BR). `load_daily_volumes()` грузит реальные объёмы AlgoPack (контракты): TICKER_LIMITS BR 75083, NG 158978, SV 84614; MAX_CONTRACTS 1000; volume cap = 10% дневного объёма. Компаунд работает до ~100M+. Paper trader реинвестирует с 10.08. Мост MT5/Finam ещё писать. Ранее: чекп.222 — базис-арбитраж Si/Eu/CNY ↔ перпетуалы (КСУР-ГО ×16, SHORT-only z>1.5, портфель OI+BA risk=8% pyr=1 → CAGR 1550%, MTM 13%, Calmar 119 после аудита); чекп.221 — OI live-эдж (60-220%/год, MDD 13-29%).
