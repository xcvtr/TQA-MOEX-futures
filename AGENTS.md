...
**Последний чекпойнт: 223 (2026-08-09)** — Live реинвест/компаунд починен: лимиты от реальных объёмов AlgoPack (BR 75083, NG 158978, SV 84614 лотов), volume cap = 10% дневного объёма, MAX_CONTRACTS 1000. **ПГО формула исправлена: ГО = medium (КСУР) × kpur/ksur** (понижение в ~2 раза, подтверждено документацией ФИНАМ «до двух раз»; было med×ksur — фейк 8.6×). ГО в PG: Si 11947, USDRUBF 11758, Eu 13897, EURRUBF 13785, CNYRUBF 958, NG 6093, SV 10509. ROI за год с ПГО: **+1801%** (без ПГО +494%), MTM 8.1%. Paper trader реинвестирует с 10.08. Мост MT5/Finam ещё писать. Ранее: чекп.222 — базис-арбитраж Si/Eu/CNY ↔ перпетуалы (портфель OI+BA risk=8% pyr=1 → CAGR 1550%, MTM 13%, Calmar 119 после аудита); чекп.221 — OI live-эдж (60-220%/год, MDD 13-29%).
...
- **Вотчер открытия позиций** — `oi_position_watchdog.py`, cron `*/5 15-23,0-4 * * 1-5` (тихий режим, молчит если ок): при НОВОЙ позиции проверяет — время входа (не будущее, торговые часы), цена vs реальная (≤0.5%), |day_net| ≥ thr на момент входа, contracts ≤ лимит
...
| **ПОРТФЕЛЬ OI LONG+SHORT** | **4 тикера** | **\|day_net\|≥3% → long/short, 60 мин, ролл-фильтр** | 35% | **+1,094%/год** | — | **14.0%** ✅ |
