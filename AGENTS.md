...
**Последний чекпойнт: 204 (2026-08-02)** — OI-стратегия BR+NG+SV, +1,042% ROI, MTM DD 20.9% (risk 15%, thr -3%, hold 60, slip 2, полные данные 64 тикера до 18.07)
...
| **ПОРТФЕЛЬ OI** | **3 тикера** | **физ продают→long, 60 мин** | 15% | **+1,042%** | — | **20.9%** ✅ |
...
- **OI (единственная enabled):** `strategies/oi/prod/engine.py` (плагин) + `fetch_day_net()` в common. BR/NG/SV, thr -3%, физ продают→long 60 мин (timeout_bars=60), risk 15%, trailing/SL отключены (0.99). **+1,042% ROI, MTM DD 20.9%** (полные данные, честная таймзона MSK→IRK+5ч)
