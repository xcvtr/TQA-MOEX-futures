...
**Последний чекпойнт: 223 (2026-08-11)** — Полный аудит + новый конфиг OI:
- **TZ-фиксы**: futoi (ISS МСК→IRK +5ч), mt5_continuous (bridge −3ч), пересборка августа
- **Аудит бэктеста**: старый включал 52% ночных сигналов (не в live) — пересчитано честно
- **Реальная ёмкость**: ISS VOLTODAY BR 813K / NG 629K / SV 415K лотов/день (tradestats_fo стух 13.07, занижал ×180!)
- **Конфиг**: thr=3, exit_thr=1.5, pyr5 (pyra_max=4, pct=0.3), stop_loss=1.5%, max_positions=1, risk=0.15 (ГО≤10%eq доминирует), LIQ_FRAC=0.05 → лимиты BR 40K/NG 31K/SV 21K
- **Бэктест (реальный компаунд)**: CAGR **+1415-2128%**, MTM DD 18.5% — 1000%/год достижимо
- **⚠️ max_positions ≠ pyr**: в live max_positions=лимит позиций на тикер, pyr — пирамидинг внутри (баг 10 позиций исправлен)
...
| Выход | day_net ≥ 1.5% (обратное условие) или 120ч |
| Стоп-лосс | **1.5%** (ключ к MDD!) |
| Пирамидинг | pyr5 (4 добавки по +0.3%) |
| Позиций на тикер | 1 (max_positions=1) |
| Риск | 10% eq (ГО≤10%eq, risk в PG 0.15 не влияет) |
| Лимиты лотов | BR 40K / NG 31K / SV 21K (5% ISS VOLTODAY) |
| Окно | 10:00-02:00 МСК (дневная + вечерняя сессия) |

**Бэктест (реальный компаунд 2023-26): CAGR +1415-2128%, MTM DD 18.5%, N=1045, WR 56%.**
