#!/usr/bin/env python3
"""Minimum viable monthly PnL tracker"""
import json, os, sys
...
for sym,df in data.items():
    ...
    for lst in PORTFOLIO.values():
        for c in lst:
            ...
            sig[k][sym]=(d.copy(),sc,di,hold,atm)
...
# Только бары когда рынок открыт
...
# Pre-build signal Series for faster access
sig_cache={}
for k,v in sig.items():
    for sym,(df,sc,di,hold,atm) in v.items():
...
for idx,ts in enumerate(all_ts):
    ...
    for lst in PORTFOLIO.values():
        for c in lst:
            ...
            ct=max(1,int(mr/go))
            if ct==0: continue
            atrv=float(data[sym].loc[ts].get('a14',0)) if 'a14' in data[sym].columns else float(data[sym].loc[ts].get('atr14',0))
...
print(f"\n{'✅ PASS' if worst[2]>-30 else '❌ FAIL'}: worst month {worst[2]:.1f}%")
