#!/usr/bin/env python3
"""
OI-wave Strategy — Grid Search for TP/SL Multipliers (H4 Timeframe).

Modified for H4:
- Resample to H4 instead of H1
- OI z-score window = 20 (on H4 ~80 hours, 3.3 days)
- Minimum wave length: 2 bars (instead of 3 on H1)
- Time-stop: 30 bars (120 hours = 5 days, same as H1's 48h on ~2-day horizon)
- Re-entry delay: 6 hours (less than 12h, since waves are rarer)
- TP/SL grid: tp_mult [1.0, 1.5, 2.0, 2.5, 3.0], sl_mult [1.0, 1.5, 2.0, 2.5, 3.0]
- Kelly 20-50%, max 3 positions, 0.01% slippage
- Period: 2025-01-01 to 2026-06-01

Portfolio: 6 tickers (GK, AF, MG, YD, SR, NR).
"""
...
ATR_PERIOD = 14
TIME_STOP_BARS = 30         # 30 H4 bars = 120 hours = 5 days
COOLDOWN_HOURS = 6          # 6 hours re-entry delay (waves are rarer on H4)
OI_Z_THRESHOLD = 2.0
MIN_WAVE_BARS = 2           # 2 bars minimum wave length (vs 3 on H1)
Z_WINDOW = 20               # 20 H4 bars ~ 3.3 days
...
def load_ticker_data(client, symbol):
    """Load 5m prices + OI for a ticker and resample to H4."""
    q = f"""
    ...
    dh = df.resample('4h').agg(agg).dropna(subset=['close'])
    if len(dh) < 100:
        print(f"  {symbol}: too few H4 bars ({len(dh)})")
        return None
    ...
    # Z-score (window=20 on H4 ~ 80 hours)
...
def detect_waves(dh):
    """Detect OI waves: |oi_z| > threshold for MIN_WAVE_BARS consecutive bars on H4."""
    waves = []
    in_wave = False
    wave_start = None
    wave_dir = None
    ...
    if in_wave and wave_streak >= MIN_WAVE_BARS:
...
def run_single_backtest(tp_mult, sl_mult, data, sym_times, all_waves, all_times):
    """
    Run a single backtest with given TP/SL multipliers on H4 data.
    ...
    for t_idx, current_time in enumerate(all_times):
        ...
        # 3) Time-stop (30 H4 bars = 120h safety net)
        for sym, pos in list(active_positions.items()):
            time_stop = pos['entry_time'] + timedelta(hours=TIME_STOP_BARS * 4)  # 30 H4 bars * 4h = 120h
        ...
        if available_positions > 0:
            for sym in data:
                ...
                for ws in wave_starts[sym]:
                    wave_time = ws['time']
                    # H4 bars are 4h apart, tolerance within 4h
                    if wave_time == current_time or (abs((wave_time - current_time).total_seconds()) <= 14400
...
def main():
    ...
    print("=" * 70)
    print("OI-Wave Strategy — Grid Search TP/SL Multipliers (H4)")
    ...
    print(f"Time-stop: {TIME_STOP_BARS} H4 bars ({TIME_STOP_BARS * 4}h)")
    print(f"Min wave: {MIN_WAVE_BARS} bars")
    print(f"Z-window: {Z_WINDOW} bars ({Z_WINDOW * 4}h)")
    ...
    for sym in TICKERS:
        ...
        if dh is not None:
            ...
            print(f"  {sym}: {len(dh_test)} H4 bars in test period")
    ...
    print(f"\nTotal H4 bars in timeline: {len(all_times)}")
    print(f"\n{'=' * 70}")
    ...
    # ── Save all results ──
    output = {
        'strategy': 'OI-wave strategy — TP/SL Grid Search (H4)',
        'tickers': TICKERS,
    ...
    out_path = os.path.join(REPORTS_DIR, 'tp_sl_grid_h4.json')
    ...
    # ── Print final table ──
    print("\n" + "=" * 70)
    print("GRID SEARCH RESULTS (sorted by Calmar) — H4")
    print("=" * 70)
    ...
    # ── Top 5 detailed ──
    print("\n" + "=" * 70)
    print("TOP 5 COMBINATIONS — DETAILED (H4)")
    print("=" * 70)
