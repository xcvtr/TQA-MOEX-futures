#!/usr/bin/env python3
"""Phase 5 5m — threshold sweep без slippage.
Смотрим как trades/day и return меняются при повышении threshold."""
...
if __name__ == '__main__':
    ...
    # Original: 0.25/0.20. Also test 0.30/0.25, 0.40/0.35, 0.50/0.45, 0.60/0.55
    thresh_sets = [
        ('0.25/0.20 (orig)', 0.25, 0.20),
        ('0.30/0.25', 0.30, 0.25),
        ('0.40/0.35', 0.40, 0.35),
        ('0.50/0.45', 0.50, 0.45),
        ('0.60/0.55', 0.60, 0.55),
    ...
    for label, long_th, short_th in thresh_sets:
        ...
        for idx, ts in enumerate(all_ts):
            ...
            for sym, pos in list(positions.items()):
                ...
                ep = None; reason = ''
                if pos['dir']=='L' and bar['low']<=pos['stop']: ep=pos['stop']; reason='stop'
                elif pos['dir']=='S' and bar['high']>=pos['stop']: ep=pos['stop']; reason='stop'
                ...
                if ep is None and 'pattern' in pos:
                    ...
                    if rs in signals_all and sk in signals_all[rs]:
                        ...
                        if ts in dfsig.index and float(dfsig.loc[ts,'score'])<0.10: ep=bar['close']; reason='fade'
                if ep is not None:
                    dm=1 if pos['dir']=='L' else -1
                    pr=dm*(ep-pos['entry'])/pos['entry']*pos['go']*pos['contracts']
                    cash+=pr; all_trades.append({'pr':pr})
            ...
            for sym, pos in positions.items():
                ...
                if rs in data_5m and ts in data_5m[rs].index:
                    bar = data_5m[rs].loc[ts]
                    mtm += (1 if pos['dir']=='L' else -1)*(bar['close']-pos['entry'])/pos['entry']*pos['go']*pos['contracts']
            teq=cash+mtm
            ...
            for lst_name, lst in PORTFOLIO.items():
                for sym, pat, di, hold, atm, w in lst:
                    ...
                    if np.isnan(score) or score<(long_th if di=='L' else short_th): continue
        ...
        for sym, pos in list(positions.items()):
            ...
            if rs in data_5m:
                ...
                pr = (1 if pos['dir']=='L' else -1)*(lb['close']-pos['entry'])/pos['entry']*pos['go']*pos['contracts']
                cash+=pr; all_trades.append({'pr':pr})
        
        tr=(cash-INITIAL_CAPITAL)/INITIAL_CAPITAL*100
        wins=sum(1 for t in all_trades if t.get('pr',0)>0)
        ...
        print(f"  DD: {max_dd*100:.1f}%  Calmar: {cal:.2f}", flush=True)
        print(f"  Trades: {total_t}  ({sigs_per_day:.1f}/day) WR:{wins/total_t*100:.1f}%" if total_t else "  0 trades", flush=True)
        
        results.append({'label':label,'return_pct':tr,'annual_return':ann*100,
    ...
    print(f"\n{'='*70}")
    print(f"{'Threshold':18} {'Return':>10} {'Ann%':>8} {'DD':>8} {'Calmar':>8} {'Trades':>8} {'/day':>6} {'WR':>6}")
    print(f"{'─'*70}")
    for r in results:
        print(f"{r['label']:18} {r['return_pct']:>+9.1f}% {r['annual_return']:>7.1f}% "
              f"{r['max_dd_pct']:>7.1f}% {r['calmar']:>8.2f} {r['n_trades']:>8} "
              f"{r['trades_per_day']:>6.1f} {r['wr']:>5.1f}%")
    
    os.makedirs('reports/tf_sweep', exist_ok=True)
    with open('reports/tf_sweep/threshold_results.json','w') as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved: reports/tf_sweep/threshold_results.json")
