#!/usr/bin/env python3
"""Phase 5 5m — compare market vs limit orders. FIXED: proper position sizing."""
...
TEST_END = '2026-04-30'
SLIP = 0.0001
FIXED_CONTRACTS = 1  # Use 1 contract per trade for clean comparison
...
def load_and_precompute():
    ...
    # Precompute signals for ALL symbols at once
    ...
    return data_5m, all_ts, signals

def simulate(data_5m, signals_all, all_ts, mode='market'):
    """mode: 'market' (0.01% slip), 'limit' (limit+fallback), 'limit_skip' (limit-only)"""
    cash = float(INITIAL_CAPITAL); peak = float(INITIAL_CAPITAL); max_dd = 0.0
    positions = {}; all_trades = []; total_slip_cost = 0.0
    pending = {}
    np.random.seed(42)
    
    # Pre-filter: for each sym/pat, build a set of timestamps where score >= threshold
    entry_cache = {}
    for sym in signals_all:
        for sk, (dfsig, di, hold, atm) in signals_all[sym].items():
    ...
    for idx, ts in enumerate(all_ts):
        ...
        if idx % 10000 == 0 and idx > 0:
            print(f"  {idx//10000+1}/7 cash={cash:.0f} pos={len(positions)} trades={len(all_trades)}", flush=True)
        
        # Process pending (limit fallback -> market on next bar)
        ...
        # MTM (simplified — only for peak tracking)
        mtm=0
        ...
        for lst_name,lst in PORTFOLIO.items():
            for sym,pat,di,hold,atm,w in lst:
                ...
                if mode == 'market':
                    ...
                    entries.append((sym, ct, ep_c, stop, go, score, di, hold))
                elif mode == 'limit':
        ...
        # Enter top 5 by score
        entries.sort(key=lambda e:e[5], reverse=True)
        for sym, ct, ep_c, stop, go, score, di, hold in entries[:5]:
            cost = ct * go
            if cost > avail: continue
            positions[sym] = {'dir':di,'hold':hold,'entry':ep_c,'stop':stop,
    ...
    return {'return_pct':round(tr,1),'annual_return':round(ann*100,1),
            ...
            'total_slip_cost':round(total_slip_cost,0)}

if __name__ == '__main__':
    data_5m, all_ts, sig = load_and_precompute()
    
    modes = [('market+0.01% slip','market'), ('limit+fallback','limit'), ('limit-only','limit_skip')]
    results = []
    for label, mode in modes:
        ...
        results.append(r)
        print(f"  Ret:{r['return_pct']:+.1f}% Ann:{r['annual_return']:+.1f}% DD:{r['max_dd_pct']:.1f}% C:{r['calmar']:.2f} Tr:{r['n_trades']}({r['trades_per_day']}/d) WR:{r['wr']}% Slip:{r['total_slip_cost']:.0f}", flush=True)
    
    print(f"\n{'='*80}")
    print(f"{'Mode':24} {'Return':>10} {'Ann%':>8} {'DD':>8} {'Calmar':>8} {'Trades':>8} {'/day':>6} {'WR':>6} {'Slip':>8}")
    print(f"{'─'*80}")
    for r in results:
        print(f"{r['label']:24} {r['return_pct']:>+9.1f}% {r['annual_return']:>7.1f}% {r['max_dd_pct']:>7.1f}% {r['calmar']:>8.2f} {r['n_trades']:>8} {r['trades_per_day']:>5.1f} {r['wr']:>5.1f}% {r['total_slip_cost']:>8.0f}")
    
    os.makedirs('reports/tf_sweep',exist_ok=True)
    with open('reports/tf_sweep/limit_order_fixed_results.json','w') as f: json.dump(results,f,indent=2)
    print(f"\nSaved reports/tf_sweep/limit_order_fixed_results.json")
