#!/usr/bin/env python3
"""Market mode ONLY — fixed ct=1, threshold 0.25/0.20. Same as market_only but from fixed.py base."""
...
# Build entry_cache (NO pre-filtering — use full signals like market_only)
entry_cache = {}
for sym in signals:
...
for idx, ts in enumerate(all_ts):
    ...
    for lst_name,lst in PORTFOLIO.items():
        for sym,pat,di,hold,atm,w in lst:
            ...
            df_ok, di_cached, hold_cached, _ = entry_cache[key]
            ...
            entries.append((sym, ct, ep_c, stop, go, score, di_cached, hold_cached))
