#!/usr/bin/env python3
"""
OI-wave Strategy — Grid Search for TP/SL Multipliers on M15.

Params:
  - Resample: 15min
  - OI z-score window: 20 (5 hours)
  - Min wave length: 4 bars (1 hour)
  - Time-stop: 96 bars (24 hours)
  - Re-entry delay: 4 hours
...
ATR_PERIOD = 14
TIME_STOP_BARS = 96         # 24 hours (96 * 15min)
COOLDOWN_BARS = 16          # 4 hours (16 * 15min)
OI_Z_THRESHOLD = 2.0
MIN_WAVE_BARS = 4           # 1 hour
Z_WINDOW = 20               # 5 hours
RESAMPLE_RULE = '15min'
...
def load_ticker_data(client, symbol):
    """Load 5m prices + OI for a ticker and resample to 15min."""
    q = f"""
    ...
    if len(dh) < 100:
        print(f"  {symbol}: too few M15 bars ({len(dh)})")
        return None
...
def run_single_backtest(tp_mult, sl_mult, data, sym_times, all_waves, all_times):
    ...
    for t_idx, current_time in enumerate(all_times):
        ...
        # 3) Time-stop (96 bars = 24h safety net)
        for sym, pos in list(active_positions.items()):
            time_stop = pos['entry_time'] + timedelta(hours=TIME_STOP_BARS * 0.25)  # 15min bars
        ...
        for sym, close_time, close_price, reason in to_close:
            ...
            cooldown_until[sym] = close_time + timedelta(hours=COOLDOWN_BARS * 0.25)
        ...
        if available_positions > 0:
            for sym in data:
                ...
                for ws in wave_starts[sym]:
                    ...
                    # Match within one bar (15min = 900s)
                    if abs(time_diff_sec) <= 900 and wave_time <= current_time:
...
def main():
    ...
    for sym in TICKERS:
        ...
        if dh is not None:
            ...
            print(f"  {sym}: {len(dh_test)} M15 bars in test period")
    ...
    print(f"\nTotal M15 bars in timeline: {len(all_times)}")
    print(f"\n{'=' * 70}")
    ...
    # ── Save all results ──
    output = {
        'strategy': 'OI-wave strategy — TP/SL Grid Search — M15',
        'tickers': TICKERS,
        'timeframe': 'M15',
    ...
    out_path = os.path.join(REPORTS_DIR, 'tp_sl_grid_m15.json')
    ...
    # ── Print final table ──
    print("\n" + "=" * 70)
    print("GRID SEARCH RESULTS — M15 (sorted by Calmar)")
    print("=" * 70)
    ...
    # ── Top 5 detailed ──
    print("\n" + "=" * 70)
    print("TOP 5 COMBINATIONS — M15 DETAILED")
    print("=" * 70)
