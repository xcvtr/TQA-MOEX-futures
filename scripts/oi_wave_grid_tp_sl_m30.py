#!/usr/bin/env python3
"""
OI-wave Strategy — Grid Search for TP/SL Multipliers on M30.

Params:
  - Resample: 30min
  - OI z-score window: 20 (10 hours)
  - Min wave length: 3 bars (1.5 hours)
  - Time-stop: 48 bars (24 hours)
  - Re-entry delay: 6 hours
...
ATR_PERIOD = 14
TIME_STOP_BARS = 48         # 24 hours (48 * 30min)
COOLDOWN_BARS = 12          # 6 hours (12 * 30min)
OI_Z_THRESHOLD = 2.0
MIN_WAVE_BARS = 3           # 1.5 hours
Z_WINDOW = 20               # 10 hours
RESAMPLE_RULE = '30min'
...
def load_ticker_data(client, symbol):
    """Load 5m prices + OI for a ticker and resample to 30min."""
    q = f"""
    ...
    dh = df.resample(RESAMPLE_RULE).agg(agg).dropna(subset=['close'])
    if len(dh) < 100:
        print(f"  {symbol}: too few M30 bars ({len(dh)})")
        return None
...
def detect_waves(dh):
    """Detect OI waves: |oi_z| > threshold for MIN_WAVE_BARS consecutive bars."""
    waves = []
    in_wave = False
    wave_start = None
    wave_dir = None
...
def run_single_backtest(tp_mult, sl_mult, data, sym_times, all_waves, all_times):
    ...
    for t_idx, current_time in enumerate(all_times):
        ...
        # 3) Time-stop (48 bars = 24h safety net)
        for sym, pos in list(active_positions.items()):
            time_stop = pos['entry_time'] + timedelta(hours=TIME_STOP_BARS * 0.5)  # 30min bars
        ...
        for sym, close_time, close_price, reason in to_close:
            ...
            cooldown_until[sym] = close_time + timedelta(hours=COOLDOWN_BARS * 0.5)
        ...
        if available_positions > 0:
            for sym in data:
                ...
                for ws in wave_starts[sym]:
                    wave_time = ws['time']
                    time_diff_sec = (current_time - wave_time).total_seconds()
                    # Match within one bar (30min = 1800s)
                    if abs(time_diff_sec) <= 1800 and wave_time <= current_time:
...
def main():
    ...
    print("=" * 70)
    print("OI-Wave Strategy — Grid Search TP/SL — M30")
    print(f"Tickers: {', '.join(TICKERS)}")
    print(f"Period: {START_TEST} to {END_TEST}")
    print(f"Resample: {RESAMPLE_RULE}, Z-window: {Z_WINDOW}, Min-wave: {MIN_WAVE_BARS} bars")
    print(f"Time-stop: {TIME_STOP_BARS} bars (24h), Cooldown: {COOLDOWN_BARS} bars (6h)")
    ...
    for sym in TICKERS:
        ...
        if dh is not None:
            ...
            print(f"  {sym}: {len(dh_test)} M30 bars in test period")
    ...
    print(f"\nTotal M30 bars in timeline: {len(all_times)}")
    print(f"\n{'=' * 70}")
    ...
    # ── Save all results ──
    output = {
        'strategy': 'OI-wave strategy — TP/SL Grid Search — M30',
        'tickers': TICKERS,
        'timeframe': 'M30',
        ...
        'parameters': {
            ...
            'time_stop_hours': TIME_STOP_BARS * 0.5,
            'cooldown_bars': COOLDOWN_BARS,
            'cooldown_hours': COOLDOWN_BARS * 0.5,
            'oi_z_threshold': OI_Z_THRESHOLD,
            'min_wave_bars': MIN_WAVE_BARS,
            'z_window': Z_WINDOW,
            'z_window_hours': Z_WINDOW * 0.5,
    ...
    out_path = os.path.join(REPORTS_DIR, 'tp_sl_grid_m30.json')
    ...
    # ── Print final table ──
    print("\n" + "=" * 70)
    print("GRID SEARCH RESULTS — M30 (sorted by Calmar)")
    print("=" * 70)
    ...
    # ── Top 5 detailed ──
    print("\n" + "=" * 70)
    print("TOP 5 COMBINATIONS — M30 DETAILED")
    print("=" * 70)
