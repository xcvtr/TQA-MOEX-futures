...
# token from env var T (short name, no dollar)
T = os.environ.get("T", "")
TABLE = os.environ.get("TABLE", "obstats")
...
PATHS = {"tradestats": ("eq", "tradestats"), "obstats": ("eq", "obstats")}


def pd(day):
    mk, ep = PATHS[TABLE]
    url = API_BASE + "/" + mk + "/" + ep + ".json?date=" + day + "&limit=100000"
    ...
    try:
        ...
        return (0, str(e))
    COL_MAP = {"tradestats": {"secid": "ticker"}, "obstats": {}}
    cn = [c.lower() for c in d["data"]["columns"]]
    cn = [COL_MAP.get(TABLE, {}).get(c, c) for c in cn]
    rows = d["data"]["data"]
    if not rows:
        return (0, None)
