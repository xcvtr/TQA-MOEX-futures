#!/usr/bin/env python3
"""Check normed yur_net waves for all tickers — full 2025-2026."""
import sys, os
...
print(f'{"Ticker":>8} | {"Bars":>7} | {"TROUGH→LONG":>12} | {"Period":>14}')
print('-' * 48)
...
for t in TICKERS:
    ...
    if len(rows) < 20:
        print(f'{t:>8} | {len(rows):>7} | {"—":>12} | NO DATA')
        continue
    ...
    status = '✅' if trough_longs > 0 else '—'
    print(f'{t:>8} | {len(rows):>7} | {trough_longs:>12} | {first}–{last}')
