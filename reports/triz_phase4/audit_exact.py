#!/usr/bin/env python3
"""Run with cap=8696 sl_pct=0.005 to match grid exactly"""
import sys, os
...
# Try different capitals to match the grid result
data = load_ticker_data('GL')
for cap_label, cap in [('cap=11765 (N=17)', 11765), ('cap=8696 (N=23)', 8696)]:
    r = backtest_one(data, 'GL', 1, 1352, vol_up_oi_up_yb_up, 13, 0.005, 0,
                     use_chandelier=True, atr_mult=2, cap_per_ticker=cap)
    if r:
        print(f"{cap_label}: ret={r['ret']:+.1f}%, net_pnl={r['net_pnl']:.0f}, tr={r['trades']}, mdd={r['mdd']:.1f}%, wr={r['wr']:.0f}%")
    else:
        print(f"{cap_label}: No trades")
