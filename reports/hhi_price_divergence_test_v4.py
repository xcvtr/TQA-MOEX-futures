#!/usr/bin/env python3
"""
HHI + Price Divergence test on MOEX futures - v4 (fixed HTTP).
"""
...
def main():
    print("=== HHI + Price Divergence Test v4 ===")
    print(f"Period: {START_DATE} to {END_DATE}")
    
    # Test connection first
    test = ch_query("SELECT 1 as ok")
    if not test:
    ...
    for ticker, rows in ticker_dates.items():
        ...
        if processed % 10 == 0:
            print(f"  [{processed}/{total}] done, {len(all_results)} signal tests so far")
    ...
    if rdf_f.empty and not rdf.empty:
        print("\nNo signals with WR >= 52%. Showing best signals:")
        best_all = rdf.loc[rdf.groupby(['asset', 'side'])['wr_pct'].idxmax()]
        best_all = best_all.sort_values('wr_pct', ascending=False).head(30)
    ...
    elif not rdf_f.empty:
        print("\n\n=== Best per asset-side (WR >= 52%) ===")
    ...
    print(f"\n\n=== All signal tests ({len(rdf)} total) ===")
