...
"""IR slippage audit — test different slippage levels."""
import sys, os
...
print('slip  ret%      cashMDD  mtmMDD   PF      tr')
for slp in [1, 2, 3, 5, 10]:
    engine = PortfolioEngine(
        [('impulse_return', ir_check, list(all_data.keys()), None)],
        broker=BrokerSim(commission=4, slippage_out=slp),
        capital=200000, slippage_in=slp)
    ...
    print(f'{slp:3d}  {ret:>+8.1f}%  {cash_mdd:>6.2f}%  {mtm_mdd:>6.2f}%  {pf:>5.2f}  {n}')
