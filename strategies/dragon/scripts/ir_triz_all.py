...
"""IR Si 1m — all TRIZ filters (20-bar hist for speed)."""
...
def run(trend=False, min_vol=0, loss_skip=0):
    reset_state()
    eq=200000.0; peak=eq; mtm_pk=eq; cdd=mdd=0; pos=None; trades=[]; cls=0
    for mi in range(60,len(bars)):
        ...
        if pos:
            ...
            if ex:
                ...
                pos=None
        if not pos:
            if loss_skip>0 and cls>=loss_skip: continue
            if trend and mi>=50:
                sma50=sum(bars[j]['prc'] for j in range(mi-50,mi))/50
                if b['prc']<sma50: continue
            if min_vol>0 and mi>=20:
                vols=[bars[j]['vol'] for j in range(mi-20,mi)]
                if sum(vols)/20<min_vol: continue
...
tests=[
    ('BASELINE',      {'trend':False,'min_vol':0,'loss_skip':0}),
    ('TREND SMA50',   {'trend':True,'min_vol':0,'loss_skip':0}),
    ('VOL>median',    {'trend':False,'min_vol':80,'loss_skip':0}),
    ('SKIP 2 loss',   {'trend':False,'min_vol':0,'loss_skip':2}),
    ('TREND+VOL',     {'trend':True,'min_vol':80,'loss_skip':0}),
    ('TREND+SKIP2',   {'trend':True,'min_vol':0,'loss_skip':2}),
    ('ALL 3',         {'trend':True,'min_vol':80,'loss_skip':2}),
]
header = f'  n    WR%   PF     ROI%   MDD%'
print(f'\n{header}')
print('-'*58)
for name,cfg in tests:
    r=run(**cfg); print(f'{name:>20s}  {r}', flush=True)
