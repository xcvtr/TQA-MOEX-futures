...
"""MTM MDD на 1 год — динамический портфель с незакрытыми позициями."""
import sys
...
TC, KNUR, START_EQ, RISK = 4, 0.5, 200000, 12
...
def select(equity):
    ...
    for t in PRIORITY:
        n = max(len(sel), 1)
        if ALL_TICKERS[t]['go'] * 2 <= go_limit / n:
            sel.append(t)
    if len(sel) < 3: sel = ['NG','SV','BR']
    return sel, {t: 1.0/len(sel) for t in sel}


def backtest():
    sel, alloc = select(START_EQ)
    bars_cache = {}
    for t in sel:
        b = load_bars(t)
        if b: bars_cache[t] = b
    
    tickers = list(bars_cache.keys())
    ...
    go_limit = START_EQ * KNUR
    ticker_eq = {t: START_EQ * alloc[t] for t in tickers}
    equity, peak_mtm = float(START_EQ), float(START_EQ)
    mtm_mdd = 0
    closed_pnl = 0
    trades = 0
    eq_peak = START_EQ
    ...
    for i in range(30, min_len):
        ...
        for t in tickers:
            ...
            if ex is not None:
                ...
                cash_mdd = max(cash_mdd, (eq_peak-equity)/eq_peak*100)
        ...
        if i % 5 == 0:
            for t in tickers:
                ...
                if not sig: continue
                risk_a = ticker_eq.get(t, equity/len(tickers)) * RISK / 100
                sl_c = sig['entry_price'] * sl / ms * sp + TC
                c = max(1, int(risk_a / sl_c)) if sl_c > 0 else 1
    ...
    print(f'  Капитал: {START_EQ:,} -> {equity:,.0f} ({ret:+.1f}%)')
    print(f'  Cash MDD (по закрытым): {cash_mdd:.2f}%')
    print(f'  MTM MDD (с открытыми):  {mtm_mdd:.2f}%')


if __name__ == '__main__':
    print(f'MTM MDD БЭКТЕСТ, risk={RISK}%, KNUR={KNUR}, ГО×0.5\n')
    backtest()
