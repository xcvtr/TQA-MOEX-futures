...
"""IR Si fast sweep — correct TF resample."""
...
rows = ch.query("SELECT bt,opn,hi,lo,prc,vol FROM moex.mt5_bars WHERE ticker='Si' AND bt>='2025-07-16' ORDER BY bt").result_rows
bars = []
for r in rows:
...
spec = {'ms':1.0,'sp':1.0,'go':6453}  # FINAM from XML MOEX
...
for tf in [1,3,5]:
    ...
    print(f'\n=== Si detect={tf}мин ({len(dbars)} bars) ===', flush=True)
    
    for rp in [1,2,3,4,5,7,10,15]:
        ...
        for mi in range(60,len(bars)):
            ...
            # Detect AFTER tick (new positions from this bar managed from NEXT bar)
            if not pos and dxi<len(dbars) and dxi not in df and mi>=d2m.get(dxi,999999999):
                ...
                if len(dh)>=20:
                    ...
                    if sig:
                        ...
                        b_vol=b['vol'] if mi<len(bars) else 999999
